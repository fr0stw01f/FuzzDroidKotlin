...
import de.tu_darmstadt.sse.appinstrumentation.transformer.*
...
class Instrumenter(private val codePositionManager: CodePositionManager,
                   private val config: DecisionMakerConfig) {


    private fun postProcessForHookingFunctionality(apkFile: String) {
        //add native code
        val instrumentedAPK = File(UtilInstrumenter.SOOT_OUTPUT_APK)
        val hookingBinaryARM = File(UtilInstrumenter.HOOKING_LIBRARY_ARM)
        val hookingBinaryX86 = File(UtilInstrumenter.HOOKING_LIBRARY_X86)

        addFilesToZip(instrumentedAPK.absolutePath,
                hookingBinaryARM.absolutePath,
                hookingBinaryX86.absolutePath)
    ...
    private fun addFilesToZip(instrumentedAPKPath: String,
                              armHookingFilePath: String,
                              x86HookingFilePath: String) {
        ...
        if (applicationName != null) {
            ...
            if (applicationSootClass != null) {
                ...
                if (attachMethod != null) {
                    ...
                    val instrumentAfterUnit: Unit = body.units.firstOrNull { it is InvokeStmt && it.invokeExpr.method.subSignature == "void attachBaseContext(android.content.Context)" } ?: throw RuntimeException("this should not happen")
