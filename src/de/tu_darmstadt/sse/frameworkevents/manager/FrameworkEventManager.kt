...
class FrameworkEventManager {
    ...
    fun extractInitalEventsForReachingTarget(targetLocation: Unit, backwardsCFG: BackwardsInfoflowCFG, manifest: ProcessManifest): MutableSet<FrameworkEvent> {
    ...
    private fun getAndroidEventsFromManifest(backwardsCFG: BackwardsInfoflowCFG, headUnits: Set<Unit>, manifest: ProcessManifest): MutableSet<FrameworkEvent> {
    ...
    private fun getDevice(devicePort: String): IDevice? {
        for (iDev in adb!!.devices) {
            if (iDev.isEmulator && iDev.serialNumber.contains(devicePort)) {
                LoggerHelper.logEvent(MyLevel.RUNTIME, "Successfully connected to emulator: " + iDev.avdName)
                return iDev
            }
        }
        return null
