...
class SMTConverter(//all source definitions
        private val sources: Set<SourceSinkDefinition>) {
    internal var smtPrograms: MutableSet<SMTProgram>? = null
    private val preparedDataFlowsForSMT: Set<ResultSourceInfo>? = null
    internal val dynamicValueInfos = HashMap<SMTProgram, Set<DynamicValueInformation>>()

    fun convertJimpleToSMT(jimpleDataFlowStatements: Array<Stmt>,
                           accessPathPath: Array<AccessPath>, targetUnits: Set<Unit>, cfg: IInfoflowCFG,
                           splitInfos: Table<Stmt, Int, MutableSet<String>>?) {
        ...
        val splitAPIElementInfos = HashBasedTable.create<List<Stmt>, Stmt, MutableList<List<String>>>()
        ...
        smtPrograms = HashSet<SMTProgram>()
        for (i in allDataFlows.indices) {
            val dataflow = allDataFlows[i]
            val accessPath = allAccessPath[i]
            val stmtVisitor = JimpleStmtVisitorImpl(sources, dataflow, accessPath, targetUnits, cfg, splitAPIElementInfos)
            try {
