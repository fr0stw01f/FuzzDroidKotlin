...
import de.tu_darmstadt.sse.decisionmaker.analysis.symbolicexecution.datastructure.*
...
class JimpleStmtVisitorImpl(sources: Set<SourceSinkDefinition>,
                            ...
                            private val targetUnits: Set<Unit>, private val cfg: IInfoflowCFG, val splitInfos: Table<List<Stmt>, Stmt, MutableList<List<String>>>) : StmtSwitch {
    ...
    private val dynamicValueInfos = HashMap<SMTProgram, MutableSet<DynamicValueInformation>>()
    ...
    fun createNewBindingForValue(value: Value): SMTBinding {
        ...
        if (hasBindingForValue(value)) {
            ...
            if (value is Local) {
                ...
                binding = SMTBinding(constantStringValue, SMTBinding.TYPE.String, 0)
            }
    ...
    fun doubleSMTPrograms() {
        val newSMTPrograms = smtPrograms
                .map { it.clone() }
                .toSet()
    ...
    override fun caseInvokeStmt(stmt: InvokeStmt) {
        ...
        if (exprVisitor.isExpressionThatNeedsToBeConvertedToSMT(invokeExpr))
            ...
            val indexOfInterest = (0..invokeExpr.argCount - 1).firstOrNull { newRhs.variableName == invokeExpr.getArg(it).toString() }
                    ?: -1
                ...
                if (unit is IdentityStmt) {
                    ...
                    if (rhs is ParameterRef) {
                        if (rhs.index == indexOfInterest) {
                            ...
                            val newLhs = createNewBindingForValue(lhs)
    ...
    override fun caseAssignStmt(stmt: AssignStmt) {
        ...
        if (rhs != null && rhs.type == SMTBinding.TYPE.Bool) {
            val lhs = createNewBindingForValue(leftOp)
    ...
    override fun caseReturnStmt(stmt: ReturnStmt) {
        ...
        val lhs = createNewBindingForValue(local)
    ...
    fun addNewDynamicValueForBaseObjectToMap(stmt: Stmt, binding: SMTBinding) {
        if (!dynamicValueInfos.keys.contains(currentSMTProgram))
            dynamicValueInfos.put(currentSMTProgram, HashSet<DynamicValueInformation>())

        val dynValueInfo = DynamicValueInformation(stmt, binding)
        dynValueInfo.isBaseObject = true

        dynamicValueInfos[currentSMTProgram]?.add(dynValueInfo)
    }
