...
object UtilSMT {
    ...
    fun removeDuplicatedFlows(allDataFlows: Set<ResultSourceInfo>): MutableSet<ResultSourceInfo> {
    ...
    fun getControlFlowPathsBetweenTwoDataFlowStmts(from: Stmt, to: Stmt, cfg: IInfoflowCFG): MutableSet<ControlFlowPath> {
        ...
        while (!worklist.isEmpty()) {
            ...
            if (cfg.isCallStmt(currentUnit)) {
                ...
                val sm = cfg.getMethodOf(currentUnit)
                for (caller in cfg.getCallersOf(sm)) {
                    cfg.getSuccsOf(caller)
                            .filter { proceedWithNextUnit(currentUnit, it, currentPaths, controlFlowPathsAtUnit) }
                            .forEach { worklist.push(it) }
                }
                continue
