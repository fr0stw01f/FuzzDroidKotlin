...
class SourceConstantFuzzer : FuzzyAnalysis() {
    ...
    override fun resolveRequest(clientRequest: DecisionRequest, completeHistory: ThreadTraceManager): List<AnalysisDecision> {
        ...
        if (completeHistory.histories.size > 1) {
            val bdLast = completeHistory.newestClientHistory!!.getProgressValue("ApproachLevel")
            val bdLastLast = completeHistory.lastClientHistory!!.getProgressValue("ApproachLevel")
            val improvedOverLastRun = bdLast < bdLastLast
            ...
            if (pinnedResponse != null) {
                ...
                val lastHistory = completeHistory.lastClientHistory
                if (lastHistory != null) {
