...
class GeneticCombination {
    ...
    fun combineGenetically(histories: Collection<ClientHistory>): ClientHistory? {
        ...
        val histIt = (histories as MutableCollection).iterator()
        ...
        while (!hists.isEmpty()) {
            ...
            if (FrameworkOptions.traceConstructionMode === TraceConstructionMode.RandomCombine || DeterministicRandom.theRandom.nextInt(MULTIPLIER) < LESSER_COMBINATION_PROBABILITY * MULTIPLIER) {
                ...
                val bestValue = hists
                        .map { it.getProgressValue("ApproachLevel") }
                        .max()
                        ?: 0
