	...
	public void send(final Collection<IClientRequest> request,
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					try{
						socket = new Socket(NetworkSettings.INSTANCE.getSERVER_IP(), NetworkSettings.INSTANCE.getSERVERPORT_OBJECT_TRANSFER());
						...
						Log.i(SharedClassesSettings.INSTANCE.getTAG(), String.format("Sending %d events to server...",
								request.size()));
						...
						// the connection
						Log.i(SharedClassesSettings.INSTANCE.getTAG(), "Waiting for server confirmation ("
...
//							Log.i(SharedClassesSettings.TAG, String.format("Received %d/%d confirmation responses", i+1, request.size()));
						...
						Log.i(SharedClassesSettings.INSTANCE.getTAG(), "All objects confirmed, closing connection...");
						...
						if (ois.read() != -1) {
							...
							Log.i(SharedClassesSettings.INSTANCE.getTAG(), "Waiting for server shutdown confirmation...");
							ois.readObject();
							Log.i(SharedClassesSettings.INSTANCE.getTAG(), "Confirmation received.");
							// We close the socket anyway
	//						if (socket.isConnected() && !socket.isClosed() && !socket.isInputShutdown())
	//							socket.shutdownInput();

							Log.i(SharedClassesSettings.INSTANCE.getTAG(), "OK, request handling done");
						}
						Log.i(SharedClassesSettings.INSTANCE.getTAG(), "Connection closed.");
					}finally {
						socket.close();
					}
				}
				...
				Log.i(SharedClassesSettings.INSTANCE.getTAG(), "End of SEND thread (" + Thread.currentThread().getId() + ").");
			}
		...
		public void run() {
			...
			try {
				try{
					...
					// Send the request
					Log.i(SharedClassesSettings.INSTANCE.getTAG(), "Sending single event to server...");
					sendRequest(oos);

					// Wait for the response
					Log.i(SharedClassesSettings.INSTANCE.getTAG(), "Waiting for single server response...");

					ServerResponse response = getResponse(ois);

					// Even in case the connection dies, take what we have and run.
					Log.i(SharedClassesSettings.INSTANCE.getTAG(), "OK, done.");
					Log.i(SharedClassesSettings.INSTANCE.getTAG(), response.toString());
	...
	//						socket.shutdownInput();
			...
			Log.i(SharedClassesSettings.INSTANCE.getTAG(), "End of CLIENT thread.");
		}
